...
void gpio_init(const struct gpio* gpio,
    const struct gpio_conf* conf)
{
    gpio_mask(gpio);
...
void gpio_set_func(const struct gpio* gpio,
    enum gpio_func func)
{
    gpio_mask(gpio);
...
void gpio_set_dir(const struct gpio* gpio,
    enum gpio_dir dir)
{
    gpio_mask(gpio);
    u32 cfg = gpio->hw->CFGR;
...
void gpio_set_pull(const struct gpio* gpio,
    enum gpio_pull pull)
{
    gpio_mask(gpio);
    u32 cfg = gpio->hw->CFGR;
...
void gpio_set_drive(const struct gpio* gpio,
